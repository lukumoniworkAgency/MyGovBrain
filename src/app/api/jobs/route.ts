...
export async function GET(request: NextRequest) {
  const limitError = await rateLimit(
    `jobs:${request.headers.get("x-forwarded-for") ?? "anonymous"}`,
    120,
  );
  ...
  if (!query.success)
    return apiError("Invalid job filters.", 422, "VALIDATION_ERROR");
  const { search, state, page, limit } = query.data;
  let requestQuery = db
    .from("jobs")
    .select(
      "id, title, organization, type, state_id, qualification, apply_url, deadline, salary, status",
    )
    .eq("status", "published")
  ...
  if (error) return apiError("Unable to load jobs.", 503, "JOB_QUERY_FAILED");
